...
§7+ 的核心原子是**梯度特征的内积**。记梯度特征 $\varphi(x)=\nabla_\theta L(x)$,则单步影响 $\Delta L(x')\approx-\eta\,\langle\varphi(x'),\varphi(x)\rangle$。下面四个经典方法,全部是 $\langle\varphi(x'),\,M\,\varphi(x)\rangle$ 的不同实例 —— **差别只在:用什么度量 $M$、累计多少 checkpoint/模型、以及如何把百万维参数空间变得可算。**

### C.1 TracIn(Pruthi et al. 2020):$M=I$,沿轨迹累加

**问题**:训练样本 $z$ 对测试样本 $z'$ 帮了还是害了?
**做法**:理想情况下追踪每次「访问 $z$」时 $z'$ 的 loss 下降,一阶即 $\Delta L(z')\approx-\eta_t\langle\nabla L(z'),\nabla L(z)\rangle$;对 $z$ 出现的步/checkpoint 求和:

$$
\mathcal I_{\text{TracIn}}(z,z')=\sum_{t\in\text{ckpts}}\eta_t\,\big\langle\nabla L(z';\theta_t),\,\nabla L(z;\theta_t)\big\rangle
$$

这**正是我们的阶梯①跨 checkpoint 求和**;单 checkpoint 版 = 末点的 GradDot = 我们脚本里的阶梯①。度量 $M=I$(把 Hessian 当单位阵),最省。符号即「协同 vs 干扰」。变体:TracInCP(选若干 ckpt)、GradDot(只末点)、GradCos(用余弦,修正大梯度离群样本)。**视角:训练动力学/轨迹**——「这一步/这次训练做了什么」。

### C.2 影响函数(Koh & Liang 2017):$M=H^{-1}$,收敛点的反事实

**问题**:若**删除/上权** $z$,收敛最优 $\theta^*$ 与测试 loss 怎么变?(不是单步,是收敛点的反事实)
**做法**:上权 $z$ 一个 $\epsilon$ → $\theta^*(\epsilon)\approx\theta^*-\epsilon H^{-1}\nabla L(z)$,链到测试 loss:

$$
\mathcal I_{\text{IF}}(z,z')=-\,\nabla L(z')^\top\,H^{-1}\,\nabla L(z),\qquad H=\nabla^2_\theta L_{\text{train}}(\theta^*)
$$

**与 TracIn 的唯一差别就是那个 $H^{-1}$**:TracIn 刻画「一次推动」(动力学),IF 刻画「新平衡落在哪」(收敛点)——$H^{-1}$ 负责把整个参数平衡的重新沉降算进去。**TracIn ≈ 取 $H=I$ 的影响函数。** 难点:$H^{-1}$ 是 $O(d^3)$ 不可行、非凸下 $H$ 可能不正定(需阻尼),实践用 CG / LiSSA 算 iHVP。**LLM 规模**:Grosse et al. 2023(Anthropic)用 **EK-FAC** 近似 $H^{-1}$,扩到 52B;发现影响稀疏、随规模抽象度上升、跨语言泛化,以及一个反直觉局限——**把关键短语顺序对调,影响几乎归零**。

### C.3 eNTK 微调(Malladi et al. 2023《A Kernel-Based View of LM Fine-Tuning》):用核回归**替代**微调

**不同的问题**:能否干脆用 $\theta_0$ 处的经验 NTK 做**核回归**来代替微调?
**做法**:线性化 $f(x;\theta)\approx f(x;\theta_0)+J_f(x;\theta_0)(\theta-\theta_0)$,则 GD 微调 = 以 $\Theta(x,x')=J_f(x)J_f(x')^\top$(eNTK)为核的核回归,且预测要**加上基座输出** $f(x;\theta_0)$(预训练初始化含任务信息,不同于随机 NTK)。**关键实证**:**加 prompt** 的微调才表现出核行为——eNTK 在多任务上逼近真实微调到 ~3%,而无 prompt 的标准微调差距可达 16%;还推导了 Adam 对应的核(依赖梯度符号),并用核视角**解释 LoRA / 子空间微调为何有效**(直接支撑 §7+.7「LoRA 更线性」)。计算用的正是**前向模式 AD 算 JVP**,与本脚本同款机器。**这是我们 ρ 很小的极端**:不止单步,整个微调都线性,于是 Regime 1 的单步预测可外推到整段训练。

### C.4 TRAK(Park et al. 2023):eNTK 线性化 + 随机投影,让影响函数可扩展

把 C.2 的 Hessian 修正 + C.3 的 eNTK 线性化 + 可扩展技巧合一:① eNTK 线性化 → 退化为逻辑/线性回归;② 用 **Johnson–Lindenstrauss 随机投影** $P\sim\mathcal N(0,1)$ 把百万维梯度特征降到低维(保内积);③ 投影空间里做 **Newton 步**(Hessian 修正,似 IF);④ 跨少量模型/ckpt **集成**;⑤ 软阈值**稀疏化**。效果:用「少数几个模型」匹配「训练上千模型(datamodels)」的归因,便宜 2–3 个数量级。**定位:把影响函数式归因,用 eNTK + 随机投影做成实用版**;随机投影是把参数空间变可算的关键招(对照我们靠「层挑选 / LoRA 子空间」降维)。

### C.5 放回我们的阶梯

| 方法 | 回答的问题 | 度量 $M$ | 累计 | 降维手段 | 对应我们的阶梯 |
|---|---|---|---|---|---|
| 阶梯①(单 ckpt) | 单步影响 | $I$ | 1 步 | 层挑选 / LoRA | = GradDot |
| **TracIn** | 轨迹影响 | $I$ | 多 ckpt | 层挑选 | 阶梯① × checkpoints |
| **影响函数** | 收敛点反事实 | $H^{-1}$ | 末点 | CG/LiSSA/EK-FAC | 阶梯① + Hessian 修正 |
| **TRAK** | 可扩展归因 | 投影 Newton | 少量模型 | JL 随机投影 | IF + eNTK + 随机投影 |
| **eNTK 微调** | 直接替代微调 | 核回归 $\Theta$ | 全程 | 前向模式 JVP | 阶梯②③ 外推到收敛 |

**三句话收束**:

1. **原子是梯度内积** $\langle\varphi(x'),M\varphi(x)\rangle$;四个方法 = 选 $M$($I$ → $H^{-1}$ → 投影 Newton)+ 选累计范围(单步 / 多 ckpt / 收敛点)+ 选降维招(层挑选 / 随机投影 / EK-FAC / LoRA 子空间)。
2. **动力学 vs 平衡**:TracIn / 我们的阶梯 = 轨迹(「这一步/这次训练做了什么」);影响函数 / TRAK = 收敛点反事实(「少了这条数据会怎样」)。问的问题不同,别混用。
3. **eNTK 微调是边界情形**:当 laziness 成立(ρ 小,尤其 prompt-based 或 LoRA),整段微调线性化,单步预测外推为「核回归替代微调」。
...
- Pruthi et al., *Estimating Training Data Influence by Tracing Gradient Descent* (TracIn), NeurIPS 2020. arXiv:2002.08484.
- Koh & Liang, *Understanding Black-box Predictions via Influence Functions*, ICML 2017. arXiv:1703.04730.
- Grosse et al., *Studying Large Language Model Generalization with Influence Functions* (EK-FAC), 2023. arXiv:2308.03296.
- Malladi et al., *A Kernel-Based View of Language Model Fine-Tuning* (eNTK), ICML 2023. arXiv:2210.05643.
- Park et al., *TRAK: Attributing Model Behavior at Scale*, ICML 2023. arXiv:2303.14186.
